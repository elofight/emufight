...
impl CpsBus {
    pub fn new(game: &'static CpsGame) -> Self {
        CpsBus {
            ...
            in1: 0xffff,
            in2: 0xffff,
            // DIP defaults match fbneo's hardware outputs (~Cpi01A/B/C):
            // DSWC 0x9F: Allow Continue=Yes(bit6=0), DemoSound=On(bit5=0), Game mode.
            // DSWB 0xFC: Normal difficulty (bits 0-2 = 0b100 → NOT = Normal).
            dswa: 0xff,
            dswb: 0xfc,
            dswc: 0x9f,
            sound_latch: 0,
