...
// Headers come from -I vendor/ymfm/src (see build.rs). Do not use ../../../ paths —
// those only work when -I happens to cancel the relative prefix (and break on MSVC).
#include "ymfm.h"
#include "ymfm_opn.h"
...
static int debug_enabled() EF_UNUSED;
...
// Web (emcc) build: no stdio diagnostics. getenv is stubbed to null so these
// never fire anyway, and emcc's float-format printf lowering pulls in libc
// symbols the minimal wasm shim doesn't provide (rust-lld undefined refs).
...
public:
    ...
    size_t adpcm_a_size() const { return m_adpcm_a.size(); }
    size_t adpcm_b_size() const { return m_adpcm_b.size(); }
...
void* neo_ym2610_create(void) {
    ...
    // Establish the documented power-on state. ymfm relies on reset() to set
    // internal register/timer state that its constructor leaves indeterminate;
    // without it the serialized chip snapshot varies between fresh instances
    // (rollback-checksum noise / potential netplay desync).
    state->chip->reset();
...
void neo_ym2610_write(void* ptr, int port, int data) {
    ...
    static uint64_t s_adpcmb_reg_writes = 0;
    static uint64_t s_adpcmb_start = 0;
    static uint16_t s_adpcmb_saddr = 0;
    static uint8_t  s_adpcmb_level = 0;
    int p = port & 3;
    s_writes[p]++;
    if (p == 0) last_addr_a = (uint8_t)data;
        ...
        // ADPCM-B (delta-t) lives in the lower address space at $10-$1B on port A.
        if (last_addr_a >= 0x10 && last_addr_a <= 0x1b) {
            s_adpcmb_reg_writes++;
            if (last_addr_a == 0x10 && (data & 0x80)) s_adpcmb_start++;   // START bit
            else if (last_addr_a == 0x12) s_adpcmb_saddr = (s_adpcmb_saddr & 0xFF00) | (uint8_t)data;
            else if (last_addr_a == 0x13) s_adpcmb_saddr = (uint16_t)((s_adpcmb_saddr & 0x00FF) | ((uint16_t)(uint8_t)data << 8));
            else if (last_addr_a == 0x1b) s_adpcmb_level = (uint8_t)data;
        }
    ...
    if (debug_enabled() && (total == 1 || total == 100 || total == 1000 || total % 5000 == 0)) {
        ...
        // std::chrono is portable (MSVC/Clang/GCC); POSIX clock_gettime is not
        // available on MSVC and breaks the Windows build.
        ...
        std::fprintf(stderr,
            "[YM2610-ymfm] ADPCM-A master=$%02X keyon[ch0..5]=%llu/%llu/%llu/%llu/%llu/%llu keyoff=%llu/%llu/%llu/%llu/%llu/%llu\n"
            "[YM2610-ymfm] ADPCM-A panvol[ch0..5]=$%02X/$%02X/$%02X/$%02X/$%02X/$%02X start[ch0..5]=%04X/%04X/%04X/%04X/%04X/%04X end0=%04X\n",
            s_adpcma_master,
            (unsigned long long)s_adpcma_on[0], (unsigned long long)s_adpcma_on[1],
            (unsigned long long)s_adpcma_on[2], (unsigned long long)s_adpcma_on[3],
            (unsigned long long)s_adpcma_on[4], (unsigned long long)s_adpcma_on[5],
            (unsigned long long)s_adpcma_off[0], (unsigned long long)s_adpcma_off[1],
            (unsigned long long)s_adpcma_off[2], (unsigned long long)s_adpcma_off[3],
            (unsigned long long)s_adpcma_off[4], (unsigned long long)s_adpcma_off[5],
            s_adpcma_panvol[0], s_adpcma_panvol[1], s_adpcma_panvol[2],
            s_adpcma_panvol[3], s_adpcma_panvol[4], s_adpcma_panvol[5],
            s_adpcma_start[0], s_adpcma_start[1], s_adpcma_start[2],
            s_adpcma_start[3], s_adpcma_start[4], s_adpcma_start[5],
            s_adpcma_end[0]);
        std::fprintf(stderr,
            "[YM2610-ymfm] ADPCM-B regwrites=%llu starts=%llu startAddr=$%04X level=$%02X (ROM A=%zu B=%zu bytes)\n",
            (unsigned long long)s_adpcmb_reg_writes,
            (unsigned long long)s_adpcmb_start,
            s_adpcmb_saddr, s_adpcmb_level,
            state->intf->adpcm_a_size(), state->intf->adpcm_b_size());
        std::fflush(stderr);
    }
