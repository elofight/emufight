...
//! # Adding a new system
//!
//! 1. Implement [`EmulatorCore`] for the new board (**including** [`RomSource`] load).
...
//! # ROM load contract
//!
//! There is **one** required load entry point: [`EmulatorCore::load_roms`] with
//! [`RomSource`]. Disk vs memory is a host concern (build a file map, or pass
//! [`RomSource::Disk`]). Cores must not leave memory load as a default error —
//! product WASM has no disk fallback.

use crate::io::InputState;
use std::collections::HashMap;

/// Where ROM bytes come from. Hosts always end up here; cores never invent paths.
#[derive(Debug, Clone, Copy)]
pub enum RomSource<'a> {
    /// Load from the host disk layout (`roms/<game>/`, system ROMs, etc.).
    Disk {
        game: &'a str,
    },
    /// Load from an in-memory basename→bytes map (WASM uploads, tests, native scan).
    Files {
        game: &'a str,
...
impl<'a> RomSource<'a> {
    pub fn game(self) -> &'a str {
        match self {
            Self::Disk { game } | Self::Files { game, .. } => game,
        }
    }

    pub fn disk(game: &'a str) -> Self {
        Self::Disk { game }
    }

    pub fn files(game: &'a str, files: &'a HashMap<String, Vec<u8>>) -> Self {
...
///
/// Required product paths (load, step, save) have **no** “not supported”
/// defaults. Optional diagnostics (`work_ram`, debug checksums) may default.
pub trait EmulatorCore: Send {
    /// Load ROMs from disk layout or an in-memory file map.
    ///
    /// Every product core (NeoGeo, CPS1, …) must implement **both**
    /// [`RomSource::Disk`] and [`RomSource::Files`]. Incomplete implementors
    /// fail to compile rather than failing at first WASM matchmake.
    fn load_roms(&mut self, source: RomSource<'_>) -> Result<(), String>;
