...
impl CpsEmulator {
    ...
    /// Install assembled ROM regions (shared by disk and in-memory load paths).
    fn apply_loaded_roms(&mut self, game: &'static CpsGame, roms: romset::CpsRoms) {
        self.game = game;
...
impl EmulatorCore for CpsEmulator {
    fn load_roms(&mut self, source: crate::core::RomSource<'_>) -> Result<(), String> {
        match source {
            crate::core::RomSource::Disk { game } => {
                let name = if game.is_empty() { "sf2" } else { game };
                let cfg = config::find(name)
                    .ok_or_else(|| format!("unsupported CPS1 game '{}'", name))?;
                let roms = romset::load(name)?;
                self.apply_loaded_roms(cfg, roms);
                Ok(())
            }
            crate::core::RomSource::Files { game, files } => {
                let cfg = config::find(game)
                    .ok_or_else(|| format!("unsupported CPS1 game '{}'", game))?;
                let roms = romset::load_from_files(game, files)?;
                self.apply_loaded_roms(cfg, roms);
                Ok(())
            }
        }
