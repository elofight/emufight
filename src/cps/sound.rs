...
impl Oki {
    ...
    fn write_command(&mut self, data: u8) {
        ...
        if self.command >= 0 {
            ...
            // Key-off: D3=ch0, D4=ch1, D5=ch2, D6=ch3 (MSM6295 datasheet).
            for v in 0..4 {
                if data & (0x08 << v) != 0 {
...
// Real ymfm OPM core on both native and wasm (see build.rs + wasm_stubs).
extern "C" {
...
impl SoundInner {
    fn read_mem(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xf002 => {
                // OKI MSM6295 status register: bit N set while voice N is playing.
                // The Z80 driver polls this in wait-loops (LD A,(F002); AND B; JR NZ,-n)
                // to sequence sounds correctly (counter narrator, speech phrases, etc.).
                let mut s = 0u8;
                for (i, v) in self.oki.voices.iter().enumerate() {
                    if v.playing {
                        s |= 1 << i;
                    }
                }
...
impl CpsSound {
    ...
    /// Run the Z80 for one video frame, stepping the YM2151 and OKI in lockstep
    /// so their state is always current when the Z80 reads hardware registers.
    ///
    /// The OKI must be advanced in lockstep (not bulk at frame-end) because the
    /// SF2CE Z80 driver polls the OKI status register (0xF002) in tight loops
    /// such as `LD A,(F002); AND B; JR NZ,-6` to wait for voices to finish
    /// before starting the next sample in a sequence (counter narrator, speech
    /// phrases, etc.).  Advancing the OKI only after the Z80 frame would leave
    /// the playing bits set forever, hanging those loops inside run_frame.
    pub fn run_frame(&mut self) {
        ...
        // OKI output-rate advancement: one 44 100 Hz sample per
        // Z80_CLOCK/OUTPUT_RATE ≈ 81.2 Z80 cycles, using a Q32 accumulator
        // to avoid drift (mirrors the YM2151 decimation model).
        const OKI_STEP: u64 = ((OUTPUT_RATE as u64) << 32) / (Z80_CLOCK as u64);
        const OKI_ONE: u64 = 1u64 << 32;
        let mut chip_accum: u64 = 0;
        let mut oki_accum: u64 = 0;
        {
            ...
            while self.cpu.cycle_count() - start < cycles_per_frame {
                ...
                oki_accum += elapsed * OKI_STEP;
                while oki_accum >= OKI_ONE {
                    oki_accum -= OKI_ONE;
                    machine.inner.oki.advance(1);
                }
                // Deliver the YM timer IRQ to the Z80 as soon as it asserts.
                self.cpu.signal_interrupt(machine.inner.ym.irq());
            }
        }
        // OKI was fully advanced in lockstep above; no bulk advance needed here.
    }
