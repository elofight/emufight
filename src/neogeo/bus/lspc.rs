...
/// 68K CPU cycles per full video frame: 264 lines × 768 cycles = 202 752,
/// i.e. 12 MHz ÷ 59.185606 Hz (the LSPC field rate). Must stay in lock-step
/// with the emulator's per-frame cycle budget so the beam/scanline register
/// and raster timer IRQ track the CPU.
pub const CYCLES_PER_BEAM_FRAME: u32 = 202_752;
/// 68K CPU cycles per scanline (= 202 752 ÷ 264 = 768, exact).
