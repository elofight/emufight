        ...
        + 'static,
{
    ...
    /// Candidate checkpoint blobs at `HOST_CHECKPOINT_INTERVAL` boundaries, keyed
    /// by frame. Overwritten in place if GGRS rolls back and resaves that frame;
    /// only promoted once `confirmed_frame()` proves no further resave can occur.
    pending_checkpoints: std::collections::BTreeMap<i32, Vec<u8>>,
    /// Frame of the last checkpoint handed out via `take_ready_checkpoint`.
    last_checkpoint_frame: i32,
}

/// Interval (frames) between confirmed-state checkpoints captured for
/// late-joining spectators. Deliberately reuses the `SaveGameState` request
/// GGRS already issues every frame for its own rollback bookkeeping — no
/// extra `step_cpu()` or serialization is added, just an occasional (once per
/// interval) clone of a blob GGRS already produced.
pub const HOST_CHECKPOINT_INTERVAL: i32 = 300; // ~5s at 60Hz
...
pub use spectator_session::{
    OnlineSpectatorSession, SeekProgress, SPECTATOR_CHECKPOINT_INTERVAL,
};
        ...
        + 'static,
{
        ...
        A: Clone + Send + Sync + std::fmt::Debug + std::hash::Hash + Eq + PartialEq + 'static,
    {
        ...
        let session = SessionBuilder::<GGRSCfg<A>>::new()
            ...
            // Pin explicitly so production stays in lock-step with ggrs_synctest_desync_check.
        ...
        Ok(Self { 
            ...
            pending_checkpoints: std::collections::BTreeMap::new(),
    ...
    pub fn advance(
        ...
        // NOTE: poll_remote_clients() is NOT called here — the caller
        // (NetplaySession::tick) already polls before each advance() call.
        // Duplicate polling would drain the socket twice per frame.
                ...
                GgrsRequest::SaveGameState { cell, frame } => {
                    match emulator.save_state_and_checksums() {
                        Ok((blob, hashes_opt)) => {
                            ...
                            // Stash a checkpoint candidate at interval boundaries — piggybacks
                            // on the save GGRS was already about to do (see `take_ready_checkpoint`).
                            if frame > 0 && frame % HOST_CHECKPOINT_INTERVAL == 0 {
                                self.pending_checkpoints.insert(frame, blob.clone());
                            }
    ...
    /// Promote the newest fully-confirmed checkpoint, if a new interval boundary
    /// has become safe since the last call. Call once per host tick alongside
    /// `pull_recent_inputs()`.
    ///
    /// A candidate at `frame` is only trustworthy once `confirmed_frame() >=
    /// frame`: GGRS never rewinds past its confirmed frame, so nothing can
    /// resave (mutate) that candidate again after this point.
    pub fn take_ready_checkpoint(&mut self) -> Option<(i32, Vec<u8>)> {
        if self.session.current_state() != ggrs::SessionState::Running {
            return None;
        }
        let confirmed = self.session.confirmed_frame();
        ...
        let frame = *self.pending_checkpoints.range(..=confirmed).next_back()?.0;
        if frame <= self.last_checkpoint_frame {
            return None;
        }
        let bytes = self.pending_checkpoints.remove(&frame)?;
        // Older boundaries are superseded — only the newest checkpoint matters.
        self.pending_checkpoints.retain(|&f, _| f > frame);
