...
/// 68k cycles per frame. The NeoGeo LSPC scans 264 lines at 768 m68k (12 MHz)
/// cycles/line = 202 752 cycles/frame → 12e6 / 202 752 = 59.185606 Hz, exactly
/// matching [`refresh_rate`]. Split into active-display (224 visible lines) +
/// vertical-blanking (40 lines) so the VBL handler receives a realistic slice.
const CYCLES_PER_FRAME: i32 = 172_032;

/// Vertical-blanking 68k cycles run after the VBL IRQ (40 lines × 768).
const CYCLES_VBLANK: i32 = 30_720;
...
impl Emulator {
    ...
    /// Load ADPCM audio samples from memory.
    pub fn load_adpcm_bytes(&mut self, adpcm_a: &[u8], adpcm_b: &[u8]) {
        self.cache_and_load_adpcm(adpcm_a, adpcm_b);
    }
    ...
    pub fn step(&mut self, n_audio_samples: usize) -> FrameOutput<'_> {
        ...
        // Produce audio.
        // Drain exactly what the chip produced this frame. At the hardware rate
        // (202 752 cyc/frame ÷ 12 MHz × 44.1 kHz ≈ 745.1 samples/frame) the count
        // alternates 745/746; the host shells resample by buffer length, so a
        // variable count stays gapless and keeps the glue ring from accumulating
        // latency. `n_audio_samples` is the nominal fallback if the ring is empty.
        self.audio_buf.clear();
        let produced = self.ym2610.ring_count();
        let n_drain = if produced > 0 { produced } else { n_audio_samples };
        self.audio.generate_samples(&mut self.audio_buf, &mut self.ym2610, n_drain);
...
impl EmulatorCore for Emulator {
    fn load_roms(&mut self, source: crate::core::RomSource<'_>) -> Result<(), String> {
        match source {
            crate::core::RomSource::Disk { game } => Emulator::load_roms(self, Some(game)),
            crate::core::RomSource::Files { game, files } => {
                memory_load::load_roms_from_memory(self, game, files)?;
                self.rom_set_name = Some(game.to_string());
                Ok(())
            }
        }
    }
