...
fn run(args: Args) -> Result<(), String> {
    ...
    emu.load_roms(emufight::RomSource::disk(&args.rom))?;
    emu.reset();
    ...
    let mut ui = SdlHost::new(args.scale, w as usize, h as usize, &args.rom, emu.refresh_rate())?;
...
struct SdlHost {
    ...
    /// SDL device output rate (Hz) obtained from the opened queue.
    device_freq: u32,
    /// Emulated field rate (Hz) used to derive each frame's true audio rate.
    emu_hz: f64,
    /// Reused resample destination to avoid per-frame allocation.
...
impl SdlHost {
    fn new(
        ...
        let audio: AudioQueue<f32> = audio_sys
            ...
            .map_err(|e| e.to_string())?;
        let device_freq = audio.spec().freq.max(1) as u32;
        audio.resume();
        ...
        Ok(Self {
            ...
            audio,
            device_freq,
            emu_hz: if emu_hz.is_finite() && emu_hz > 0.0 { emu_hz } else { 60.0 },
    ...
    fn queue_audio(&mut self, samples: &[f32]) {
        ...
        // Avoid unbounded growth if the device is stuck (~1 s at device rate).
        if self.audio.size() > self.device_freq {
            return;
        }
        // The core renders one field of audio (`samples.len()` samples) per
        // emulated frame, so its true rate is `len * emu_hz`. NeoGeo runs a
        // 60fps/44.1kHz sample budget (735/frame) but is paced at 59.19 Hz, so
        // that true rate (~43.5kHz) is below the SDL device rate — feeding it
        // raw underruns and crackles. Resample from the true rate to the device
        // rate so production matches consumption exactly (gapless).
        let src_rate = (samples.len() as f64 * self.emu_hz).round().max(1.0) as u32;
        if src_rate == self.device_freq {
            let _ = self.audio.queue_audio(samples);
            return;
        }
        resample_linear(samples, src_rate, self.device_freq, &mut self.resample_buf);
        let _ = self.audio.queue_audio(&self.resample_buf);
    }
}

/// Linear-interpolate `src` (at `src_rate` Hz) into `out` (at `dst_rate` Hz).
/// Mono f32. `out` is cleared and refilled.
fn resample_linear(src: &[f32], src_rate: u32, dst_rate: u32, out: &mut Vec<f32>) {
    out.clear();
