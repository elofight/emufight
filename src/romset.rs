...
pub fn collect_adpcm_roms_for(game_name: &str) -> (Vec<u8>, Vec<u8>) {
    ...
    // On NeoGeo the V ROMs are wired to BOTH the YM2610 ADPCM-A and the
    // ADPCM-B (delta-t) sample buses, addressing the same physical ROM data.
    // Games such as kof98 trigger delta-t samples (voices / effects); mirror the
    // concatenated V ROM into the ADPCM-B region so those reads return real data
    // instead of silence.
    b = a.clone();
    (a, b)
}
