...
**WASM:** the same ymfm sources are compiled with **Emscripten** (`emcc` / `emar` on `PATH`, e.g. `brew install emscripten`) so native and web share one chip core (required for rollback netplay). Minimal C/C++ shims live in `src/wasm_stubs.rs`.
